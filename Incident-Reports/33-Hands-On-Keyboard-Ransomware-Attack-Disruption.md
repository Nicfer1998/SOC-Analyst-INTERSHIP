...
| **Dispositivos afectados** | `[hostname-dc1].[org-domain]` (DC), `veeambackup` |
...
### 8. Movimiento lateral a servidor de backups Veeam

**01:43 AM — 01:51 AM** *(después de la contención, posiblemente con sesión previa activa)*

El atacante se movió al servidor `veeambackup` (Windows Server 2012 R2, en Workgroup — fuera del dominio):
...
| Eliminar cuenta local `server` en `veeambackup` | Equipo IT cliente | ✅ Recomendado |
| Resetear cuenta `krbtgt` x2 (intervalo 10 horas) | Equipo IT cliente | ✅ Recomendado |
| Verificar integridad de backups en Veeam | Equipo IT cliente | ✅ Recomendado |
...
| Implementar segmentación de red para servidor Veeam | Equipo IT cliente | ⚠️ Urgente |
...
3. **El servidor de backups es siempre un objetivo pre-ransomware** — destruir los backups antes de cifrar la producción elimina la capacidad de recuperación sin pagar el rescate. Veeam debe estar segmentado de la red de producción.
